...
let URL = ring.create({
    ...
    isAnyHost() {
        return Constants.ANYHOST_VALUE === this.host || URL.Boolean(this.parameters[Constants.ANYHOST_KEY], false);
    },
    getServiceInterface() {
    ...
    getMethodParameter(method, key, defaultValue) {
        let value = this.parameters[method + "." + key];
        if (_.isEmpty(value)) {
            if (!_.isUndefined(defaultValue)) return defaultValue;
            value = this.parameters[key];
            if (_.isEmpty(value)) {
                value = this.parameters[Constants.DEFAULT_KEY_PREFIX + key];
            }
            return value;
        }
        return value;
 ...
 * @return {boolean}
 */
URL.Boolean = function(value, defaultValue) {
    return Boolean(_.isUndefined(value) ? defaultValue : value);
};
