...
const URL = require('./URL.ring');
const NotifyListener = require('./NotifyListener.ring');
const RegistryService = require('./RegistryService.ring');
...
let AbstractRegistry = RegistryService.$extend({
    ...
    filterEmpty(url, urls) {
        if (urls == null || urls.length == 0) {
            url.protocol = Constants.EMPTY_PROTOCOL;
            return [this];
        }
        return urls;
    },
    loadProperties() {
        if (this.file && fs.existsSync(this.file)) {
            try {
                ...
                this.logger && this.logger.info("Load registry store file " + this.file + ", data: " + result);
                this.properties = JSON.parse(result);
            } catch (err) {
                if (this.logger)
                    this.logger.warn('Failed to load registry store file %s', this.file, err);
                else
        ...
        co(function * () {
            ...
            if (categoryNotified != null) {
                for (let us of categoryNotified.values()) {
                    console.log(yield url.toFullString());
    ...
    recover() {
        ...
        if (this.registered.size > 0) {
            ...
            for (let url of this.registered.values()) {
                this.register(url);
            }
        }
        // subscribe
    ...
    destroy() {
        if (this.logger){
            this.logger.info("Destroy registry:%s", this.registryUrl);
        }
        if (this.registered.size > 0) {
            for (let url of this.registered.values()) {
