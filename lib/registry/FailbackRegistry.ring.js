...
const AbstractRegistry = require('./AbstractRegistry.ring');
const Constants = require('../Constants');
const SkipFailbackWrapperException = require('../error/SkipFailbackWrapperException.ring');
const URL = require('./URL.ring.js');
let FailbackRegistry = AbstractRegistry.$extend({
    name: 'FailbackRegistry',
...
function checkUrl(url) {
    return URL.Boolean(this.registryUrl.parameters[Constants.CHECK_KEY], true)
        && URL.Boolean(url.parameters[Constants.CHECK_KEY], true)
        && !Constants.CONSUMER_PROTOCOL == url.protocol;
}
function isCheckOrSkipFailback(url, e) {
    let check = checkUrl.call(this, url);
